#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
Versão refatorada: caminhos genéricos (BASE_DIR / data | logs | results) e
imports enxutos. Lógica original preservada.
"""

import sys
import os
import shutil
import random
import time
from math import sqrt
from datetime import datetime
from pathlib import Path

import numpy as np
from numpy import concatenate
from matplotlib import pyplot
from pandas import read_csv, DataFrame
from sklearn.preprocessing import MinMaxScaler, LabelEncoder
from sklearn.metrics import mean_squared_error
from scipy import stats
from tensorflow import keras
from tensorflow.keras import layers
from keras_tuner.tuners import RandomSearch

# ────────────────────────────── PATHS ──────────────────────────────
BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))         # pasta do script
LOGS_DIR     = os.path.join(BASE_DIR, "logs")                       			   # *.txt finais de médias
RESULTS_ROOT = os.path.join(BASE_DIR, "results", "LSTM")            			   # subpastas por execução
DATA_DIR = os.path.join(BASE_DIR, "data")                                      # execuções

for d in (DATA_DIR, LOGS_DIR, RESULTS_ROOT):
    d.mkdir(parents=True, exist_ok=True)

# ─────────────────── Argumento: nº de repetições ───────────────────
try:
    valor_inteiro = int(sys.argv[1])
except (IndexError, ValueError):
    print("Passe um número inteiro como argumento.")
    sys.exit(1)

# ─────────────────── Variáveis de tempo médio ───────────────────
media1 = media4 = media5 = media6 = 0
aux = 0
nomelogmedia = datetime.now()

# ─────────────────────────── LOOP PRINCIPAL ─────────────────────────
while aux < valor_inteiro:

    # pasta da execução individual (timestamp)
    run_dir = RESULTS_ROOT / datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    run_dir.mkdir(parents=True, exist_ok=True)

    # ────────────── 1. Manipulação dos dados ──────────────
    with open("ManDados.txt", "a", encoding="utf-8") as lg:
        lg.write(f"------ Manipulação dos dados iniciada em: {datetime.now()}\n\n")
    inicio = time.time()

    dataset_path = DATA_DIR / "e1_Treino.csv"
    dataset1 = read_csv(dataset_path, header=0, index_col=0, delimiter=";")
    values1 = dataset1.values
    values1 = values1[~np.isnan(values1).any(axis=1)].astype("float32")

    scaler = MinMaxScaler()
    scaled1 = DataFrame(scaler.fit_transform(values1)).values

    n_train = 36
    train1, test1 = scaled1[:n_train, :], scaled1[n_train:, :]
    train_X1, train_y1 = train1[:, :-1], train1[:, -1]
    test_X1, test_y1 = test1[:, :-1], test1[:, -1]

    train_X1 = train_X1.reshape((-1, 1, train_X1.shape[1]))
    test_X1 = test_X1.reshape((-1, 1, test_X1.shape[1]))

    t1 = time.time() - inicio
    with open("ManDados.txt", "a", encoding="utf-8") as lg:
        lg.write(f"------ Manipulação dos dados encerrada em: {datetime.now()}\n")
        lg.write(f"Tempo de execução: {t1}\n")
    shutil.move("ManDados.txt", run_dir / "ManDados.txt")

    # ────────────── 2. Random Search (Keras Tuner) ──────────────
    with open("RS.txt", "a", encoding="utf-8") as lg:
        lg.write(f"------ Random Search iniciada em: {datetime.now()}\n\n")
    inicio = time.time()

    def build_model(hp):
        model = keras.Sequential()
        model.add(
            layers.LSTM(
                units=hp.Int("lstm_units", 1, 512, step=1),
                activation=hp.Choice("lstm_activation", ["relu", "tanh"]),
                return_sequences=True,
                input_shape=(1, 33),
            )
        )
        model.add(
            layers.LSTM(
                units=hp.Int("lstm_units_1", 1, 512, step=1),
                activation=hp.Choice("lstm_activation", ["relu", "tanh"]),
            )
        )
        if hp.Boolean("dropout"):
            model.add(layers.Dropout(0.25))
        model.add(layers.Dense(1, activation="sigmoid"))
        model.compile(
            optimizer=keras.optimizers.Adam(
                learning_rate=hp.Float("lr", 1e-4, 1e-2, sampling="log")
            ),
            loss="binary_crossentropy",
            metrics=["accuracy"],
        )
        return model

    tuner_dir = run_dir / "KT"
    tuner = RandomSearch(
        build_model,
        objective="val_accuracy",
        max_trials=10,
        executions_per_trial=3,
        directory=tuner_dir,
        project_name="p20_infestado",
    )

    tuner.search(train_X1, train_y1, epochs=5, validation_data=(test_X1, test_y1))
    best_model = tuner.get_best_models(1)[0]

    t4 = time.time() - inicio
    with open("RS.txt", "a", encoding="utf-8") as lg:
        lg.write(f"------ Execução encerrada em: {datetime.now()}\n")
        lg.write(f"Tempo de execução: {t4}\n")
    shutil.move("RS.txt", run_dir / "RS.txt")

    # salvar summary do best_model
    with open(run_dir / "modelsummary.txt", "w") as f:
        best_model.summary(print_fn=lambda x: f.write(x + "\n"))

    # ────────────── 3. Treinamento completo ──────────────
    with open("TreinoMA.txt", "a", encoding="utf-8") as lg:
        lg.write(f"------ Treinamento iniciado em: {datetime.now()}\n\n")
    inicio = time.time()

    history = best_model.fit(
        train_X1, train_y1, epochs=5000, validation_data=(test_X1, test_y1)
    )

    t5 = time.time() - inicio
    with open("TreinoMA.txt", "a", encoding="utf-8") as lg:
        lg.write(f"------ Execução total encerrada em: {datetime.now()}\n")
        lg.write(f"Tempo de execução: {t5}\n")
    shutil.move("TreinoMA.txt", run_dir / "TreinoMA.txt")

    # ────────────── 4. Métricas & Gráficos ──────────────
    with open("CalcCKT.txt", "a", encoding="utf-8") as lg:
        lg.write(f"------ Cálculos iniciados em: {datetime.now()}\n\n")
    inicio = time.time()

    yhat1 = best_model.predict(test_X1)
    test_X1_flat = test_X1.reshape(test_X1.shape[0], test_X1.shape[2])

    inv_yhat1 = scaler.inverse_transform(concatenate((test_X1_flat, yhat1), axis=1))[:, -1]
    inv_y1 = scaler.inverse_transform(
        concatenate((test_X1_flat, test_y1.reshape(-1, 1)), axis=1)
    )[:, -1]

    rmse = sqrt(mean_squared_error(inv_y1, inv_yhat1))
    desvio_pred = np.std(inv_yhat1)
    variancia_pred = inv_yhat1.var()
    desvio_real = np.std(inv_y1)
    variancia_real = inv_y1.var()
    r_value = stats.linregress(inv_y1, inv_yhat1).rvalue

    coeffs1 = np.polyfit(inv_y1, inv_yhat1, 5)
    r1 = (
        np.sum((np.poly1d(coeffs1)(inv_y1) - inv_yhat1.mean()) ** 2)
        / np.sum((inv_yhat1 - inv_yhat1.mean()) ** 2)
    )

    t6 = time.time() - inicio
    with open("CalcCKT.txt", "a", encoding="utf-8") as lg:
        lg.write(f"------ Cálculos encerrados em: {datetime.now()}\n")
        lg.write(f"Tempo de execução: {t6}\n")
    shutil.move("CalcCKT.txt", run_dir / "CalcCKT.txt")

    # Boxplot
    erro = inv_yhat1 - inv_y1
    fig1, _ = pyplot.subplots()
    pyplot.boxplot([inv_y1, inv_yhat1, erro], labels=["Real", "Predito", "Erro"])
    pyplot.title("P20 - Infestado")
    pyplot.savefig(run_dir / "GBoxP_CKT.png", dpi=fig1.get_dpi() * 2)
    pyplot.close()

    # Dispersão
    pyplot.scatter(inv_yhat1, inv_y1)
    pyplot.xlim(0, 110)
    pyplot.ylim(0, 110)
    pyplot.plot(
        [inv_y1.min(), inv_yhat1.max()], [inv_y1.min(), inv_yhat1.max()], "red"
    )
    pyplot.title("P20 Infestado - Real x Predito")
    pyplot.ylabel("Real")
    pyplot.xlabel("Predito")
    pyplot.savefig(run_dir / "GDisp_CKT.png", dpi=fig1.get_dpi() * 2)
    pyplot.close()

    # log detalhado
    with open(run_dir / "Log_CKT.txt", "w", encoding="utf-8") as file:
        file.write("P20 - Infestado\n")
        file.write(f"Test RMSE: {rmse:.3f}\n")
        file.write(f"R2 linear: {r_value ** 2}\n")
        file.write(f"R2 Polinomial: {r1}\n")
        file.write(f"Desvio Real: {desvio_real}\n")
        file.write(f"Variancia Real: {variancia_real}\n")
        file.write(f"Desvio Predito: {desvio_pred}\n")
        file.write(f"Variancia Predito: {variancia_pred}\n")
        file.write("Real\n" + ", ".join(f"{a:.2f}" for a in inv_y1) + "\n")
        file.write("Predito\n" + ", ".join(f"{b:.2f}" for b in inv_yhat1) + "\n")

    # acumula tempos médios
    media1 += t1
    media4 += t4
    media5 += t5
    media6 += t6
    aux += 1

# ─────────────────────── Métricas Médias Gerais ───────────────────────
media1 /= valor_inteiro
media4 /= valor_inteiro
media5 /= valor_inteiro
media6 /= valor_inteiro

with open(
    LOGS_DIR / f"log_medias_{nomelogmedia.strftime('%Y-%m-%d_%H-%M-%S')}.txt",
    "a",
    encoding="utf-8",
) as lg:
    lg.write(f"A média de tempo de manipulação dos dados é: {media1}s\n\n")
    lg.write(f"A média de tempo de execução da Random Search é: {media4}s\n\n")
    lg.write(f"A média de tempo de execução do treinamento com KT é: {media5}s\n\n")
    lg.write(f"A média de tempo dos cálculos com KT é: {media6}s\n\n")
