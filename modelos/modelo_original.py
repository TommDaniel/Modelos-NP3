...
"""
Refatoração: caminhos centralizados (BASE_DIR / data | logs | results),
imports enxutos e n_train dinâmico = len(arquivo) - 12.
"""
...
# ─────────────────────── PATHS ───────────────────────
BASE_DIR     = Path(__file__).resolve().parent.parent   # um nível acima
DATA_DIR     = BASE_DIR / "data"
LOGS_DIR     = BASE_DIR / "logs"
RESULTS_ROOT = BASE_DIR / "results" / "LSTM"
...
# n_train dinâmico → total de linhas do arquivo menos 12
...
model = Sequential([
    LSTM(44, input_shape=(train_X1.shape[1], train_X1.shape[2]), return_sequences=True),
    LSTM(22),
    Dense(1)
])
...
r1              = (np.sum((np.poly1d(coeffs1)(inv_y1) - inv_yhat1.mean()) ** 2) /
